...
class Registration extends Simulation {
	...
	val scn = scenario("Registration")
		...
		.exec(http("request_1")
			...
			.get("/fsxstorefront/fsx/en_US/cart/rollover/MiniCart?_=1563877188813")
			.headers(headers_2))
			.check(status.is(404)))
		.pause(1)
		.exec(http("request_3")
		...
		.exec(http("request_5")
			...
			.check(status.is(404)),
            http("request_8")
			.get("/fsxstorefront/fsx/en_US/cart/rollover/MiniCart?_=1563877202016")
			.headers(headers_2),
            http("request_9")
			.get("/fsxstorefront/_ui/responsive/common/fonts/glyphicons-halflings-regular.ttf")
			.check(status.is(404))))
		...
		.exec(http("request_10")
			...
			.formParam("email", "fsx2@gmail.com")
